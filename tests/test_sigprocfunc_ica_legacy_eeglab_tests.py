...
def test_icadefs_current_suite_has_platform_binary_and_sampling_defaults():
    defaults = icadefs()
...
@eeglab_test(
    f"{SIGPROC_ROOT}/kmeanscluster/sigprocfunc_kmeanscluster_wrapperTest.m",
    "test_test_kmeanscluster",
)
def test_kmeanscluster_current_suite_exact_example_and_high_dimensional_case():
...
def test_posact_current_suite_rectangular_weights_exact_outputs():
...
def test_posact_current_suite_explicit_sphere_exact_outputs():
    data = np.asarray([[-1, 0, 1, 2], [5, -2, 3, -4], [0, 1, -1, 0]], dtype=float)
    weights = np.asarray([[1, 2, -1], [-3, -4, 10]], dtype=float)
...
def test_posact_current_suite_square_weights_and_all_negative_component():
    data = np.asarray([[-1, 0, 1, 2], [5, -2, 3, -4], [0, 1, -1, 0]], dtype=float)
...
def test_runica_ml2_current_suite_corrects_miswired_upstream_call():
...
def test_runica_mlb_current_suite_delegates_to_qualified_engine():
    _assert_variant_matches_runica(
        runica_mlb,
