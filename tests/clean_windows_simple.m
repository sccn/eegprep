...
% for each channel...
X = signal.data(1,:).^2;
X = sum(X(bsxfun(@plus,offsets,wnd')));
wz = zeros(C, length(X));
for c = C:-1:1
    ...
    [mu,sig] = fit_eeg_distribution_simple(X, ...
...
function [mu, sig] = fit_eeg_distribution_simple2(X, varargin)
    % Example of proper initialization
    mu = 0; % Initialize `mu` with a default value
    sig = 0; % Initialize `sig` with a default value
