...
class TestEegLat2PointParity(unittest.TestCase):
    ...
    def test_parity_continuous(self):
        ...
        epoch_array = []  # continuous

        py_out, py_flag = eeg_lat2point(lat_array, epoch_array, srate, timewin, 1.0)
        ml_out, ml_flag = self.eeglab.eeg_lat2point(lat_array, epoch_array, srate, timewin, 1.0)

        self.assertTrue(np.allclose(py_out, ml_out, atol=1e-9))
        self.assertEqual(int(py_flag), int(ml_flag))
    ...
    def test_parity_milliseconds(self):
        ...
        ml_out, ml_flag = self.eeglab.eeg_lat2point(lat_array, epoch_array, srate, timewin, 1e-3)
...
class TestEegLat2PointFunctional(unittest.TestCase):
    ...
    def test_eeg_lat2point_continuous(self):
        # 100 Hz, epoch 0–1 s → 101 points; 0,0.5,1.0 s map to 1,51,101
        srate = 100
        timewin = [0, 1]
    ...
    def test_eeg_lat2point_milliseconds(self):
        # ms output: 0 ms and 1000 ms → points 1 and 1001 at 1000 Hz
        srate = 1000
        timewin = [0, 1]  # seconds
    ...
    def test_eeg_lat2point_outrange_clamp(self):
        ...
        self.assertEqual(int(out[0]), 101)  # max valid point
