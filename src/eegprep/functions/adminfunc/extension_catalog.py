"""Metadata-only curated extension catalog support."""
...
class ExtensionCatalog:
    ...
    def by_name(self) -> dict[str, ExtensionCatalogEntry]:
        ...
        return {_normalize_name(entry.name): entry for entry in self.entries}

    def by_package(self) -> dict[str, ExtensionCatalogEntry]:
        """Return entries keyed by normalized Python package name."""
        return {_normalize_name(entry.package_name): entry for entry in self.entries if entry.package_name}
