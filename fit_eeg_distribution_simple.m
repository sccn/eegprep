function [mu,sig] = fit_eeg_distribution_simple(X,min_clean_fraction,max_dropout_fraction,quants,step_sizes,beta)
...
if any(step_sizes<0.0001) || any(step_sizes>0.1) || numel(step_sizes) ~= 2
...
max_width = quants(2)-quants(1);                   % maximum width is the fit interval if all data is clean
min_width = min_clean_fraction*max_width(1);   % minimum width of the fit interval, as fraction of data

% get matrix of shifted data ranges
range1 = lower_min:step_sizes(1):lower_min+max_dropout_fraction;
range2 = 1:round(n(1) * max_width(1));
X = X(round(bsxfun(@plus, range2', round(n * range1))));
X1 = X(1,:); X = bsxfun(@minus,X,X1);

opt_val = Inf;
% for each interval width...
range3 = round(n*(max_width(1):-step_sizes(2):min_width));
opt_lu = [ 0 0 ];
opt_bounds = [0 0];
opt_beta = 0;
for mInd = 1:length(range3)
    m = range3(mInd);
...
alpha = (opt_lu(2)-opt_lu(1))/(opt_bounds(2)-opt_bounds(1));
mu = double(opt_lu(1)-opt_bounds(1)*alpha);
beta = opt_beta;
