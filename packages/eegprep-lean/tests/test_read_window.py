...
class TestAHostTransport:
    """Through the package's top-level names, as a host reaches them."""

    def test_a_registered_transport_serves_a_read_that_names_none(self, served, host_fetch) -> None:
        ...
        index, store, _ = served
        eegprep_lean.set_default_transport(eegprep_lean.FetchTransport(host_fetch))
    ...
    def test_an_explicit_transport_still_wins(self, served, host_fetch) -> None:
        """Registering a default must not capture a caller who named their own."""
        index, store, requested = served
        eegprep_lean.set_default_transport(eegprep_lean.FetchTransport(host_fetch))
        requested.clear()

        read(index, store, start_sample=0, n_samples=2, physical=False, transport=UrllibTransport(timeout_s=10))

        assert host_fetch.seen == [], "the registered default served a read that named its own transport"
        assert requested, "and the named one never reached the server"
